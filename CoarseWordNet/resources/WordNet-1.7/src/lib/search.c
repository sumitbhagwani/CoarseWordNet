...
static char *Id = "$Id: search.c,v 1.145 2001/06/19 15:01:22 wn Exp $";
...
IndexPtr parse_index(long offset, int dbase, char *line) {
    ...
    if (!strcmp(wnrelease, "1.6") || !strcmp(wnrelease, "1.7")) {
...
static void tracenomins(SynsetPtr synptr, int dbase)
{
    ...
    for (i = 0; i < synptr->ptrcount; i++) {
	if ((synptr->ptrtyp[i] >= NOMIN_START) &&
	    (synptr->ptrtyp[i] <= NOMIN_END)) {
...
unsigned int is_defined(char *searchstr, int dbase)
{
	...
	for(i = 0; i < index->ptruse_cnt; i++) {
	    ...
	    if (index->ptruse[i] >= NOMIN_START &&
		 index->ptruse[i] <= NOMIN_END) {
		retval |= bit(NOMINALIZATIONS);
	    }
